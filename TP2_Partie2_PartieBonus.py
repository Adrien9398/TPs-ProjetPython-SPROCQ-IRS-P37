...
def exporter_csv():
    ...
    with open('resultats_ssh.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=';')
        writer.writerow(['IP', 'Tentatives échouées', 'Tentatives réussies'])
        for ip in ips_liste:
            writer.writerow([ip, compteur_echec.get(ip, 0), compteur_success.get(ip, 0)])
    print("Résultats exportés dans le fichier 'resultats_ssh.csv'.")

def menu():
    while True:
        print("\n=== MENU ===")
        print("1. Afficher les 5 IPs avec le plus d'échecs")
        print("2. Comparer échecs et réussites par IP")
        print("3. Exporter les résultats en CSV")
        print("4. Quitter")
